classdef epiStimulationOffline < handle
%  Define regions of stimulation from epi-fluorescence arm
%
% Select in the live preview image a region of 
% interest to be illuminated by the DMD setup

    properties
        
        Folder = 'C:\Karthala\Kardinal\calibration_stack_3\';
        FolderCalib = 'C:\Users\LJP\Documents\DMD_Calib_Antoine_250122.tif';%UPDATE AH 250122
    ...
    methods
        ...
        function app = epiStimulationOffline
        ...
        function uiMaker(app, hObjects, eventData)
            ...
            uicontrol(app.ControlsDMD,'String', 'Save MaskedImage',...
            ...
            uicontrol(app.ControlsDMD,'String', 'Save Mask',...
                'Callback', @app.saveMask,...
...
%             app.Mask(263:262+499, 133:132+499) = 0;  % 500 x 500 square
        ...
        function calibrateDMD(app, hObjects, eventData)
            ...
            app.ImageCamera = imread([app.FolderCalib]);
            figure(app.CameraFigure)
        ...
        function createDmdMask(app, hObjects, eventData)
            ...
            M = tempMask'; 
            %M = fliplr(flipud(M)); %UPDATE FROM AH 250122 ok horizontally
            M = fliplr(tempMask.');
            
% full height square case    
            disp(app.DmdPosition)
...
%             'OutputView','full'); % Translate right (add left band)
        ...
        function saveMask(app, hObjects, eventData)
            Mask = app.Mask;
            save('mask.mat', 'Mask');           
        end        
