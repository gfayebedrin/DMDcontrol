...
class StimDMDWidget(QWidget):
    """Orchestrate the full DMD stimulation workflow.

    The widget is responsible for loading camera imagery, configuring a DMD
    calibration, allowing the user to draw regions of interest (ROIs), and
    sequencing stimulation patterns.  It stitches together the specialised
    helper classes found in :mod:`stim1p.ui.capture_tools` and
    :mod:`stim1p.ui.calibration_preferences` while exposing a single, cohesive
    Qt widget to the rest of the application.
    """
    ...
    def _connect(self):
        ...
        Centralising the signal/slot wiring makes it easy to audit how user
        actions are propagated through the widget and keeps the ``__init__``
        method focused on layout concerns.
        """
    ...
    def _rotation_matrix(self, angle: float | None = None) -> np.ndarray:
        """Return the 2x2 rotation matrix for the supplied angle.

        Args:
            angle: Angle in radians.  When ``None`` the currently defined axis
                angle is used.

        Returns:
            ``numpy.ndarray`` containing the rotation matrix that maps camera
            pixels into the axis frame.
        """
    ...
    def _camera_to_axis(
        ...
        *,
        ...
        Args:
            points: Positions expressed in camera pixels.  Accepts either a
                single ``(x, y)`` pair or an ``(N, 2)`` array.
            origin: Optional camera-space origin from which to measure the
                axis.  Defaults to the persisted axis origin.
            angle: Optional rotation override in radians.  Defaults to the
                persisted axis angle.

        Returns:
            ``numpy.ndarray`` in the axis coordinate system with the same
    ...
    def _axis_to_camera(
        ...
        *,
        ...
        Args:
            points: Axis-space coordinates to transform.  Accepts either a
                single point or an array of points.
            origin: Optional axis origin in camera space.
            angle: Optional rotation override in radians.

        Returns:
            ``numpy.ndarray`` containing the coordinates expressed in camera
            pixels.
        """
        arr = np.asarray(points, dtype=float)
        was_1d = arr.ndim == 1
    ...
    def _axis_origin_micrometre(
        ...
        """Return the current axis origin expressed in micrometres."""
        if self._calibration is None:
    ...
    def _axis_pixels_to_micrometres(self, points: np.ndarray) -> np.ndarray:
        """Transform axis-space coordinates into physical micrometres."""
        if self._calibration is None:
            raise RuntimeError("A calibration is required for micrometre conversion.")
        arr = np.asarray(points, dtype=float)
        was_1d = arr.ndim == 1
    ...
    def _micrometres_to_axis_pixels(self, points_um: np.ndarray) -> np.ndarray:
        """Convert micrometre coordinates into the axis frame for drawing."""
        if self._calibration is None:
            raise RuntimeError("A calibration is required for micrometre conversion.")
    ...
    def _update_image_transform(self) -> None:
        """Align the displayed camera image with the defined axis."""
    ...
    def _image_axis_bounds(self) -> tuple[float, float, float, float]:
        """Return the extremal coordinates of the image in axis space."""
    ...
    def _update_axis_visuals(self) -> None:
        """Refresh the overlay items that visualise the coordinate axis."""
    ...
    def _fit_view_to_image(self, *, use_axis: bool = True) -> None:
        """Scale the view box to keep the current image fully visible."""
    ...
    def _set_axis_state(
        ...
        """Persist the axis parameters and update any dependent visuals."""
    ...
    def _apply_axis_definition(self, origin_camera: np.ndarray, angle_rad: float) -> None:
        """Commit a new axis definition and migrate existing ROIs.

        Existing ROIs are stored relative to the previous axis definition.  To
        keep them visually stable, the points are first transformed back into
        camera space and then reprojected into the freshly defined axis frame.
        """
        ...
        if existing_shapes:
            ...
            for item, (axis_points, shape_type) in existing_shapes.items():
                ...
                # Reproject from the previous axis definition into camera space so
                # the shapes can be reinterpreted under the new axis parameters.
    ...
    def _set_image(
        ...
        *,
        ...
        Args:
            image: Raw camera frame expressed as a 2-D grayscale or 3-D RGB
                ``numpy.ndarray``.
            fit_to_view: When ``True`` the view range is recomputed to keep the
                entire image visible.
            apply_axis: When ``True`` the image is reoriented according to the
                defined calibration axis.
            auto_contrast: When ``True`` automatically compute sensible display
                levels instead of reusing the previously selected histogram
                region.
        """
