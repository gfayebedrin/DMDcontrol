...
class InteractiveRectangleCapture(QObject):
    ...
    def __init__(self, view_box: pg.ViewBox, parent: QWidget | None = None):
        """Initialise the capture helper with the view box to observe."""
    ...
    def eventFilter(self, _obj, event):  # noqa: D401 - Qt signature
        """Handle the mouse gestures that define the rectangle."""
    ...
    def _finish(self, rect: QRectF | None) -> None:
        """Store ``rect`` as the result and exit the modal event loop."""
        self._result = rect
        self._dragging = False
    ...
    def _cleanup_rect(self) -> None:
        """Remove any temporary preview geometry from the scene."""
        if self._rect_item is not None:
...
class AxisCapture(QObject):
    ...
    def __init__(self, view_box: pg.ViewBox, parent: QWidget | None = None):
        """Initialise the helper with the view box it should monitor."""
        super().__init__(parent)
        self._view_box = view_box
        self._scene = view_box.scene()
        self._loop: QEventLoop | None = None
    ...
    def eventFilter(self, _obj, event):  # noqa: D401
        """Handle mouse interaction to define an axis with live feedback."""
        if self._loop is None:
            return False
        etype = event.type()
        if etype == QEvent.GraphicsSceneMousePress:
    ...
    def _ensure_preview_items(self) -> None:
        """Create the temporary graphics items used for visual feedback."""
    ...
    def _update_preview(self, current: QPointF) -> None:
        """Update the preview geometry to track the cursor position."""
        if self._origin_view is None:
            return
        self._ensure_preview_items()
    ...
    def _finish(self, cancel: bool) -> None:
        """Store the result and exit the modal event loop."""
    ...
    def _cleanup_preview(self) -> None:
        """Remove any preview items from the view box."""
...
class PolygonDrawingCapture(QObject):
    ...
    def __init__(self, view_box: pg.ViewBox, parent: QWidget | None = None):
        """Initialise the helper with its target view box."""
        super().__init__(parent)
        self._view_box = view_box
        self._scene = view_box.scene()
        self._loop: QEventLoop | None = None
    ...
    def eventFilter(self, _obj, event):  # noqa: D401
        """Handle mouse gestures to build up the polygon vertices."""
        if self._loop is None:
            return False
        etype = event.type()
        if etype == QEvent.GraphicsSceneMousePress:
    ...
    def _append_point(self, scene_pos: QPointF) -> None:
        """Add the clicked position to the polygon vertex list."""
    ...
    def _update_preview(self, current: QPointF | None) -> None:
        """Refresh the polyline preview as the user moves the cursor."""
    ...
    def _finish(self, commit: bool) -> None:
        """Persist the polygon when ``commit`` is True and exit the loop."""
